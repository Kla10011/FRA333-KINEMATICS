...
class Trajectory_publisher(Node):
    ...
    def subscription_callback(self,msg):
        ...
        if self.is_cal:
            ...
            # for i in gamma:
            #     # q, flag  = inverse_kinematics([0.67-self.pos_init[0], self.pos_init[1], 0.3-self.pos_init[2]] , i)

            #     q, flag  = inverse_kinematics([abs(0.67-self.pos_init[0]),self.pos_init[1], abs(0.3-self.pos_init[2])] , i)
            #     if flag == True: # can do this task space
            #         self.keep_l = self.pos_init.copy()
            #         # self.kepp_l = [0.67+self.pos_init[0], self.pos_init[1], 0.3+self.pos_init[2]]
            #         self.goal_positions = [q[0], q[1], q[2]]
            #         self.cur_pos = self.goal_positions.copy()
            #         # print('True')

            #         break

            # if flag == False:
            #     self.goal_positions = self.cur_pos.copy()
            #     # self.pos_init = self.cur_pos.copy()
            #     self.pos_init = self.keep_l.copy()
            #     # self.pos_init[0] -= self.cur_pos[0]
            #     # self.pos_init[1] -= self.cur_pos[1]
            #     # self.pos_init[2] -= self.cur_pos[2]




            #     print('False')
            # print('pos init : ', self.pos_init)
            # # print('self.cur_pos :',self.cur_pos)
