// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from romeona_interfaces:srv/EnableTrack.idl
// generated code does not contain a copyright notice
#include "romeona_interfaces/srv/detail/enable_track__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

// Include directives for member types
// Member `s`
#include "std_msgs/msg/detail/bool__functions.h"

bool
romeona_interfaces__srv__EnableTrack_Request__init(romeona_interfaces__srv__EnableTrack_Request * msg)
{
  if (!msg) {
    return false;
  }
  // s
  if (!std_msgs__msg__Bool__init(&msg->s)) {
    romeona_interfaces__srv__EnableTrack_Request__fini(msg);
    return false;
  }
  return true;
}

void
romeona_interfaces__srv__EnableTrack_Request__fini(romeona_interfaces__srv__EnableTrack_Request * msg)
{
  if (!msg) {
    return;
  }
  // s
  std_msgs__msg__Bool__fini(&msg->s);
}

bool
romeona_interfaces__srv__EnableTrack_Request__are_equal(const romeona_interfaces__srv__EnableTrack_Request * lhs, const romeona_interfaces__srv__EnableTrack_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // s
  if (!std_msgs__msg__Bool__are_equal(
      &(lhs->s), &(rhs->s)))
  {
    return false;
  }
  return true;
}

bool
romeona_interfaces__srv__EnableTrack_Request__copy(
  const romeona_interfaces__srv__EnableTrack_Request * input,
  romeona_interfaces__srv__EnableTrack_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // s
  if (!std_msgs__msg__Bool__copy(
      &(input->s), &(output->s)))
  {
    return false;
  }
  return true;
}

romeona_interfaces__srv__EnableTrack_Request *
romeona_interfaces__srv__EnableTrack_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  romeona_interfaces__srv__EnableTrack_Request * msg = (romeona_interfaces__srv__EnableTrack_Request *)allocator.allocate(sizeof(romeona_interfaces__srv__EnableTrack_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(romeona_interfaces__srv__EnableTrack_Request));
  bool success = romeona_interfaces__srv__EnableTrack_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
romeona_interfaces__srv__EnableTrack_Request__destroy(romeona_interfaces__srv__EnableTrack_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    romeona_interfaces__srv__EnableTrack_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
romeona_interfaces__srv__EnableTrack_Request__Sequence__init(romeona_interfaces__srv__EnableTrack_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  romeona_interfaces__srv__EnableTrack_Request * data = NULL;

  if (size) {
    data = (romeona_interfaces__srv__EnableTrack_Request *)allocator.zero_allocate(size, sizeof(romeona_interfaces__srv__EnableTrack_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = romeona_interfaces__srv__EnableTrack_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        romeona_interfaces__srv__EnableTrack_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
romeona_interfaces__srv__EnableTrack_Request__Sequence__fini(romeona_interfaces__srv__EnableTrack_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      romeona_interfaces__srv__EnableTrack_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

romeona_interfaces__srv__EnableTrack_Request__Sequence *
romeona_interfaces__srv__EnableTrack_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  romeona_interfaces__srv__EnableTrack_Request__Sequence * array = (romeona_interfaces__srv__EnableTrack_Request__Sequence *)allocator.allocate(sizeof(romeona_interfaces__srv__EnableTrack_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = romeona_interfaces__srv__EnableTrack_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
romeona_interfaces__srv__EnableTrack_Request__Sequence__destroy(romeona_interfaces__srv__EnableTrack_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    romeona_interfaces__srv__EnableTrack_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
romeona_interfaces__srv__EnableTrack_Request__Sequence__are_equal(const romeona_interfaces__srv__EnableTrack_Request__Sequence * lhs, const romeona_interfaces__srv__EnableTrack_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!romeona_interfaces__srv__EnableTrack_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
romeona_interfaces__srv__EnableTrack_Request__Sequence__copy(
  const romeona_interfaces__srv__EnableTrack_Request__Sequence * input,
  romeona_interfaces__srv__EnableTrack_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(romeona_interfaces__srv__EnableTrack_Request);
    romeona_interfaces__srv__EnableTrack_Request * data =
      (romeona_interfaces__srv__EnableTrack_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!romeona_interfaces__srv__EnableTrack_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          romeona_interfaces__srv__EnableTrack_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!romeona_interfaces__srv__EnableTrack_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
romeona_interfaces__srv__EnableTrack_Response__init(romeona_interfaces__srv__EnableTrack_Response * msg)
{
  if (!msg) {
    return false;
  }
  // structure_needs_at_least_one_member
  return true;
}

void
romeona_interfaces__srv__EnableTrack_Response__fini(romeona_interfaces__srv__EnableTrack_Response * msg)
{
  if (!msg) {
    return;
  }
  // structure_needs_at_least_one_member
}

bool
romeona_interfaces__srv__EnableTrack_Response__are_equal(const romeona_interfaces__srv__EnableTrack_Response * lhs, const romeona_interfaces__srv__EnableTrack_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // structure_needs_at_least_one_member
  if (lhs->structure_needs_at_least_one_member != rhs->structure_needs_at_least_one_member) {
    return false;
  }
  return true;
}

bool
romeona_interfaces__srv__EnableTrack_Response__copy(
  const romeona_interfaces__srv__EnableTrack_Response * input,
  romeona_interfaces__srv__EnableTrack_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // structure_needs_at_least_one_member
  output->structure_needs_at_least_one_member = input->structure_needs_at_least_one_member;
  return true;
}

romeona_interfaces__srv__EnableTrack_Response *
romeona_interfaces__srv__EnableTrack_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  romeona_interfaces__srv__EnableTrack_Response * msg = (romeona_interfaces__srv__EnableTrack_Response *)allocator.allocate(sizeof(romeona_interfaces__srv__EnableTrack_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(romeona_interfaces__srv__EnableTrack_Response));
  bool success = romeona_interfaces__srv__EnableTrack_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
romeona_interfaces__srv__EnableTrack_Response__destroy(romeona_interfaces__srv__EnableTrack_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    romeona_interfaces__srv__EnableTrack_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
romeona_interfaces__srv__EnableTrack_Response__Sequence__init(romeona_interfaces__srv__EnableTrack_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  romeona_interfaces__srv__EnableTrack_Response * data = NULL;

  if (size) {
    data = (romeona_interfaces__srv__EnableTrack_Response *)allocator.zero_allocate(size, sizeof(romeona_interfaces__srv__EnableTrack_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = romeona_interfaces__srv__EnableTrack_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        romeona_interfaces__srv__EnableTrack_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
romeona_interfaces__srv__EnableTrack_Response__Sequence__fini(romeona_interfaces__srv__EnableTrack_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      romeona_interfaces__srv__EnableTrack_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

romeona_interfaces__srv__EnableTrack_Response__Sequence *
romeona_interfaces__srv__EnableTrack_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  romeona_interfaces__srv__EnableTrack_Response__Sequence * array = (romeona_interfaces__srv__EnableTrack_Response__Sequence *)allocator.allocate(sizeof(romeona_interfaces__srv__EnableTrack_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = romeona_interfaces__srv__EnableTrack_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
romeona_interfaces__srv__EnableTrack_Response__Sequence__destroy(romeona_interfaces__srv__EnableTrack_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    romeona_interfaces__srv__EnableTrack_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
romeona_interfaces__srv__EnableTrack_Response__Sequence__are_equal(const romeona_interfaces__srv__EnableTrack_Response__Sequence * lhs, const romeona_interfaces__srv__EnableTrack_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!romeona_interfaces__srv__EnableTrack_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
romeona_interfaces__srv__EnableTrack_Response__Sequence__copy(
  const romeona_interfaces__srv__EnableTrack_Response__Sequence * input,
  romeona_interfaces__srv__EnableTrack_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(romeona_interfaces__srv__EnableTrack_Response);
    romeona_interfaces__srv__EnableTrack_Response * data =
      (romeona_interfaces__srv__EnableTrack_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!romeona_interfaces__srv__EnableTrack_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          romeona_interfaces__srv__EnableTrack_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!romeona_interfaces__srv__EnableTrack_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
